...
// ── useLabelCatalog ──────────────────────────────────────────────────────────
/**
 * Catálogo de etiquetas con estrategia híbrida (misma que doctores/servicios):
...
export function useLabelCatalog(includeArchived = false) {
  ...
  const [serverResults, setServerResults] = useState<Label[] | null>(null);

  const pageRef = useRef(0);
  const debouncedQuery = useDebouncedValue(query, SEARCH_DEBOUNCE_MS);

  const isComplete = total <= labels.length;

  const fetchPage = useCallback(
    async (page: number, append: boolean) => {
      setLoading(true);
      try {
        const { entities, pagination } = await labelsService.getLabelsPage({
          includeArchived,
          page,
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const { entities } = await labelsService.getLabelsPage({
          query: trimmed,
          includeArchived,
          page: 0,
          pageSize: CATALOG_PAGE_SIZE,
  ...
  const results = useMemo(() => {
    if (isComplete) {
      ...
      return labels.filter((l) => matchesQuery(l.name, trimmed));
    }
    if (!query.trim()) return labels;
    return serverResults ?? labels;
  }, [isComplete, labels, query, serverResults]);
