...
export class TreatmentSuggestionService {
  static generateSuggestions(
    ...
    this.addPulpalStatusSuggestions(pulpalStatus, procedureCatalog, suggestions)
  ...
  private static addPulpalStatusSuggestions(
    ...
    if (pulpalStatus === "irreversible" || pulpalStatus === "necrosis") {
      const endo = catalog.find((p) => p.id === "endo-unirradicular")
      if (endo) {
        suggestions.push({
          procedure: endo,
      ...
      const nucleo = catalog.find((p) => p.id === "nucleo")
      if (nucleo) {
        suggestions.push({
          procedure: nucleo,
      ...
      const corona = catalog.find((p) => p.id === "corona-metal-ceramica")
      if (corona) {
        suggestions.push({
          procedure: corona,
  ...
  private static addICDASSuggestions(
    ...
    if (incipient.length > 0) {
      ...
      if (oclusales.length > 0) {
        const sellante = catalog.find((p) => p.id === "sellante")
        if (sellante) {
          suggestions.push({
            procedure: sellante,
      ...
      if (proximales.length > 0) {
        const infiltracion = catalog.find((p) => p.id === "infiltracion")
        if (infiltracion) {
          suggestions.push({
    ...
    if (moderate.length > 0) {
      const resina = catalog.find((p) => p.id === "resina-simple")
      if (resina) {
        suggestions.push({
          procedure: resina,
    ...
    if (severe.length > 0) {
      const resinaCompleja = catalog.find((p) => p.id === "resina-compleja")
      if (resinaCompleja) {
        suggestions.push({
      ...
      if (severe.length >= 3) {
        const onlay = catalog.find((p) => p.id === "onlay")
        if (onlay) {
          suggestions.push({
            procedure: onlay,
