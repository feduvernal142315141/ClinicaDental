import type { ClinicalEvent, ToothSurface } from "@/components/odontogram/types"
import { ODONTOGRAM_STATE_COLORS, COLOR_PRIORITY_ORDER, type ColorPriority } from "../constants/odontogram-colors.constants"
...
export class OdontogramColorService {
  ...
  static getSurfaceColor(
    ...
    const relevantEvents = allEvents.filter(e => 
      ...
      (e.surfaces.includes(surface) || e.surfaces.length === 0 || e.level === "tooth")
    )
   ...
   */
  static getToothColor(toothNumber: number, allEvents: ClinicalEvent[]): string {
  ...
  private static getEventPriority(event: ClinicalEvent): ColorPriority {
    ...
    if (event.type === "plan" && event.priority === "alta" && event.status !== "done") {
    ...
    if (event.type === "plan" && (event.status === "plan" || event.status === "in_progress")) {
      return "planned"
    }
   ...
   */
  static getToothEventsSummary(toothNumber: number, allEvents: ClinicalEvent[]) {
    const toothEvents = allEvents.filter(e => e.toothNumber === toothNumber)
   ...
   */
  static hasActiveDiagnosis(toothNumber: number, allEvents: ClinicalEvent[]): boolean {
   ...
   */
  static hasPendingTreatments(toothNumber: number, allEvents: ClinicalEvent[]): boolean {
    return allEvents.some(e => 
      e.toothNumber === toothNumber && 
      e.type === "plan" && 
      e.status === "plan"
   ...
   */
  static getToothUrgency(toothNumber: number, allEvents: ClinicalEvent[]): "none" | "low" | "medium" | "high" | "emergency" {
    const toothEvents = allEvents.filter(e => e.toothNumber === toothNumber)
    
    // Emergencia: ICDAS 6 o prioridad alta + dolor
    const hasEmergency = toothEvents.some(e => 
      (e.icdasScore === 6) || 
    ...
    const hasHigh = toothEvents.some(e => 
      e.icdasScore === 5 || 
      e.type === "endo"
    )
    ...
    // Medio: ICDAS 3-4
    const hasMedium = toothEvents.some(e => 
      e.icdasScore && e.icdasScore >= 3
    )
    if (hasMedium) return "medium"

    // Bajo: ICDAS 1-2 o observación
    const hasLow = toothEvents.some(e => 
