...
export class ToothSymbolService {
   ...
   */
  static getToothSymbol(toothNumber: number, allEvents: ClinicalEvent[]): string | null {
    ...
    const hasEndo = toothEvents.some(e => 
      e.type === "endo" || 
    ...
    const hasCrown = toothEvents.some(e => 
      (e.category === "protesis" || e.procedureName?.toLowerCase().includes("corona")) && 
    ...
    // 5. PUENTE - B
    const hasBridge = toothEvents.some(e => 
    ...
    const hasRestoration = toothEvents.some(e => 
    ...
    const hasPreventive = toothEvents.some(e => 
   ...
   */
  static getAllToothSymbols(toothNumber: number, allEvents: ClinicalEvent[]): Array<{
    symbol: string
    description: string
    ...
    if (toothEvents.some(e => e.type === "endo" || e.category === "endodoncia")) {
      symbols.push({
    ...
    if (toothEvents.some(e => e.category === "protesis" && (e.type === "performed" || e.status === "done"))) {
      symbols.push({
    ...
    if (toothEvents.some(e => e.category === "restaurador" && (e.type === "performed" || e.status === "done"))) {
      symbols.push({
    ...
    if (toothEvents.some(e => e.category === "preventivo" && (e.type === "performed" || e.status === "done"))) {
      symbols.push({
   ...
   */
  static shouldShowSymbol(toothNumber: number, allEvents: ClinicalEvent[]): boolean {
  ...
  static getSymbolColor(backgroundColor: string): string {
    ...
    const darkBackgrounds = ["#424242", "#D32F2F", "#B91C1C", "#991B1B", "#9C27B0", "#1E88E5", "#EF5350", "#F44336"]
